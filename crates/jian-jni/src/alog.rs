...
//!
//! Binds `liblog`'s `__android_log_write` directly so the JNI layer can emit
//! the paired window-ownership lines and diagnostics the Task 7 acceptance
//! script asserts via `adb logcat`, without pulling in a `log`-facade backend
//! that would need runtime initialization.
