//! Lightweight markdown parser — block structure (H3/H4 headings, bullet
//! lists, paragraphs) plus inline styling (`**bold**`, `` `code` ``, `#HEX`
//! colour chips). Pure (no `Painter`): callers render the [`MdBlock`] /
//! [`MdRun`] sequence themselves, so the same parser serves a one-shot
//! Markdown component or a panel that interleaves blocks with its own chrome.
...
/// Parse a markdown content string into renderable blocks. Content longer than
/// `limit` chars is truncated with an ellipsis.
pub fn parse_blocks(content: &str, limit: usize) -> Vec<MdBlock> {
...
/// Split one inline-markdown line into styled runs. Recognises `**bold**`,
/// `` `code` `` and `#RRGGBB` colour tokens; everything else is plain text.
pub fn parse_inline(line: &str) -> Vec<MdRun> {
...
/// Find an `open … close`-delimited span; returns `(start, end, run)` in byte
/// offsets where `end` is just past the closing delimiter.
...
/// Greedy word-wrap a run sequence to `max_chars` per line. Plain / bold runs
/// break at spaces; code / colour runs stay whole.
pub fn wrap_runs(runs: &[MdRun], max_chars: usize) -> Vec<Vec<MdRun>> {
...
/// Append `text` to `line` as the same style as `style`, merging with a
/// trailing run of the same style where possible.
